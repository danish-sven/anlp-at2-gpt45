...
def main():
    ...
    all_languages = language if args.l1 is None else [args.l1]

    df = pd.DataFrame(columns=['original', 'corrected', 'native_lang'])

    for l1_lang in all_languages:
        with codecs.open(args.data_path, 'r', encoding='utf8') as f:
            for line in f:
                data_num += 1
                try:
                    jsonData = json.loads(line, strict=False)
                    l2_langs, native_lang = jsonData[2], jsonData[3]
                    orig_sents, corr_sents = jsonData[4], jsonData[5]
                    if (l1_lang == native_lang) and args.l2 in l2_langs:
                            ...
                            if len(corr_sents[i]) > 0:
                                ...
                                for corr_sent in corr_sents[i]:
                                    ...
                                    if not tag_err and text != "":
                                        df = df.append({'original': orig_sent, 'corrected': text, 'native_lang': l1_lang}, ignore_index=True)
                            else:
                                df = df.append({'original': orig_sent, 'corrected': "Uncorrected", 'native_lang': l1_lang}, ignore_index=True)
                except:
                    error_num += 1
                    pass
    ...
    print(args.l1)
    print(args.l2)
    df.to_csv("2. Raw Data/languages_to_" + args.l2 + ".csv", index=False)
