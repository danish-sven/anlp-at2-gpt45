...
def save_to_csv(l1, l2, outputs):
    ...
    with open(filename, mode='w', encoding='utf-8', newline='') as csvfile:
        ...
        csv_writer.writerow(['original', 'correct'])
        for output in outputs:
            orig, corr = output.split('\t')
            csv_writer.writerow([orig, corr])
    ...
    with codecs.open(args.data_path, 'r', encoding='utf8') as f:
        for line in f:
            try:
                ...
                if l1_lang in language and any(l2_lang in language for l2_lang in l2_langs):
                    outputs = make_sent_pair(orig_sents, corr_sents, args)
